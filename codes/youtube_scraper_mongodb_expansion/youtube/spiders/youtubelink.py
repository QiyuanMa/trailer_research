...
class YoutubespiderSpider(Spider):
    ...
    live_url = "https://www.youtube.com/playlist?list=PLScC8g4bqD461pCfNojDx0fVxjGl7Og_4"
    ...
    def start_requests(self):
        yield Request( self.live_url,callback=self.parse_live)

    def parse_live(self, response):
        print(response.text)
        selector = Selector(response)
        videolinks = selector.xpath('//text()').extract()
        print(videolinks)
        # for i in range(len(videolinks)):
        #     link=videolinks[i]
        #     mid=videomids[i]
        #     # print(link)
        #     videolink = VideoLinksItem()
        #     videolink["_id"]=mid
        #     videolink["url"]="https://weibo.com"+link
        #     videolink["mid"]=mid
        #     yield videolink
        # end_id = videomids[-1]
        # yield Request(self.type_url.format(type=type,page=page,end_id=end_id),
        #               callback=self.parse_Types
        #               ,meta={'page': 2, 'type': type})
