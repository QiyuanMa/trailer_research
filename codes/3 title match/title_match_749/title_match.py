# The program uses re.match() to merge the trailer_info table and the movie_info table into one table, according to the movie title. 
import json
import re
from title_match_749.datamanage_joblomovietrailer import DataManager
db = DataManager()
def load_trailer():
    with open(r'C:\Users\student\Desktop\movie_trailer_merged_info.json','r', encoding='utf8') as trailer:
...
if __name__ == "__main__":
    ...
    for i in range(len(title)):
        for j in range(1,len(Title)):
            if re.match(Title[j],title[i])!= None:
                #print(len(re.match(Title[j],title[i]).group()))
                if title[i][len(re.match(Title[j],title[i]).group())]!=':' and title[i][len(re.match(Title[j],title[i]).group())+1]!='I' and \
                        title[i][len(re.match(Title[j],title[i]).group())+1]!='2' and title[i][len(re.match(Title[j],title[i]).group())+1]!='3'\
                        and title[i][len(re.match(Title[j],title[i]).group())]!='4' and title[i][len(re.match(Title[j],title[i]).group())+1]!='5' and title[i][len(re.match(Title[j],title[i]).group())+1]!='6':
